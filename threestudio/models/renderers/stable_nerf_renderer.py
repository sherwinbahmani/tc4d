...
class PatchRenderer(VolumeRenderer):
    ...
    def configure(
        ...
        **kwargs,
        ...
        if hasattr(self.base_renderer, "renderers"):
    ...
    def forward(
        ...
        nerf_to_render_idx: Optional[int] = None,
        **kwargs,
    ) -> Dict[str, Float[Tensor, "..."]]:
        B, H, W, _ = rays_o.shape
        valid_patch_key = []
        out_global = {}
        if nerf_to_render_idx is None:
            renderer = self.base_renderer
        else:
            renderer = self.base_renderer.renderers[nerf_to_render_idx]
