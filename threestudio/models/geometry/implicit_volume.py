...
class ImplicitVolume(BaseImplicitGeometry):
        ...
        if self.density_grid is None:
            ...
            with torch.no_grad():
                density = self.density_network(
                    self.encoding(verts.reshape(-1, self.cfg.n_input_dims), self.density_grid)
                ).reshape(*verts.shape[:-1], 1)

                _, density = self.get_activated_density(verts, density)
                self.density_grid = density.reshape(64, 64, 64).unsqueeze(0).detach()
    ...
    def forward(
        ...
        enc, dx, elastic_loss, divergence_loss, dx_shifted = self.encoding(points.view(-1, self.cfg.n_input_dims), self.density_grid, out_all=True)
    ...
    def forward_density(self, points: Float[Tensor, "*N Di"]) -> Float[Tensor, "*N 1"]:
        ...
        density = self.density_network(
            self.encoding(points.reshape(-1, self.cfg.n_input_dims), self.density_grid)
    ...
    def export(self, points: Float[Tensor, "*N Di"], **kwargs) -> Dict[str, Any]:
        ...
        points_unscaled = points
        points = contract_to_unisphere(points_unscaled, self.bbox, self.unbounded)
        enc = self.encoding(points.reshape(-1, self.cfg.n_input_dims), self.density_grid)
        features = self.feature_network(enc).view(
            *points.shape[:-1], self.cfg.n_feature_dims
        )
        out.update(
