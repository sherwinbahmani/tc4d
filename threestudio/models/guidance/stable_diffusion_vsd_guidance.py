...
class StableDiffusionVSDGuidance(BaseModule):
    ...
    def configure(self) -> None:
        ...
        # self.compute_grad_vsd_t = None
        # self.compute_grad_vsd_noise = None
        # self.train_lora_t = None
        # self.train_lora_noise = None
        # self.train_lora_camera_condition = None
    ...
    def compute_grad_vsd(
        ...
        with torch.no_grad():
            # random timestamp
            # if self.compute_grad_vsd_t is None:
            #     t = torch.randint(
            #         self.min_step,
            #         self.max_step + 1,
            #         [B],
            ...
            #         device=self.device,
            #     )
            #     self.compute_grad_vsd_t = t
            # else:
            #     t = self.compute_grad_vsd_t
            t = torch.randint(
                self.min_step,
                self.max_step + 1,
                [B],
            ...
            # add noise
            # if self.compute_grad_vsd_noise is None:
            #     noise = torch.randn_like(latents)
            #     self.compute_grad_vsd_noise = noise
            # else:
            #     noise = self.compute_grad_vsd_noise
            noise = torch.randn_like(latents)
    ...
    def train_lora(
        ...
        # if self.train_lora_noise is None:
        #     noise = torch.randn_like(latents)
        #     self.train_lora_noise = noise
        # else:
        #     noise = self.train_lora_noise
        noise = torch.randn_like(latents)
        ...
        # if self.train_lora_camera_condition is None:
        #     if self.cfg.lora_cfg_training and random.random() < 0.1:
        #         camera_condition = torch.zeros_like(camera_condition)
        #     self.train_lora_camera_condition = camera_condition
        # else:
        #     camera_condition = self.train_lora_camera_condition
        if self.cfg.lora_cfg_training and random.random() < 0.1:
            camera_condition = torch.zeros_like(camera_condition)
