...
class RandomMultiviewCameraIterableDataset(RandomCameraIterableDataset):
    ...
    def collate(self, batch) -> Dict[str, Any]:
        ...
        if self.cfg.sample_rand_frames == "fps_t0":
            fps = torch.FloatTensor(1).uniform_(0, 1/self.cfg.num_frames).item()
            t0 = torch.FloatTensor(1).uniform_(0, 1-self.cfg.num_frames*fps).item()
            frame_times = torch.linspace(t0, t0+fps*self.cfg.num_frames, (self.cfg.num_frames+1))[:self.cfg.num_frames]
        elif self.cfg.sample_rand_frames == "t0":
            t0 = torch.FloatTensor(1).uniform_(0, 1/self.cfg.num_frames).item()
            frame_times = torch.linspace(t0, t0+(self.cfg.num_frames-1)/self.cfg.num_frames, self.cfg.num_frames)
