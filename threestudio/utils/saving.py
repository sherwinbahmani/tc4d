...
class SaverMixin:
    ...
    def get_image_grid_(self, imgs, align):
        ...
        cols = []
        for col in imgs:
            assert col["type"] in ["rgb", "uv", "grayscale", "rgba"]
            if col["type"] == "rgb":
    ...
    def save_image_grid(
        ...
        if video:
            ...
            # imageio.mimsave(self.get_save_path(filename.replace(".png", ".mp4")), img_video, fps=4)
            torchvision.io.write_video(self.get_save_path(filename.replace(".png", ".mp4")), np.array(img_video), fps=fps)
    ...
    def save_img_sequence(
        ...
        if save_format == "gif":
            ...
            torchvision.io.write_video(self.get_save_path(filename), np.array(imgs), fps=fps)
